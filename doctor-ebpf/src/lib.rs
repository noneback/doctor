...
use doctor_common::{StackInfo, EVENT_TRACE_ALWAYS, EVENT_TRACE_NEW};
...
pub unsafe fn collect_trace<C: EbpfContext>(ctx: C) {
    ...
    let stack_info = StackInfo {
        tgid: tgid,
        pid: pid,
        user_stack_id: Some(user_stack_id),
        kernel_stack_id: Some(kernel_stack_id),
        cmd: cmd,
        cpu: cpu,
    };
    ...
    if let Some(count) = COUNTS.get_ptr_mut(&stack_info) {
        *count += 1;
        ...
        // update hashmap with count and and only push new stack_infos to queue for
        // symbol resolution
